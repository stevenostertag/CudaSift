...
void CudaImage::Allocate(int w, int h, int p, bool host, float *devmem, const float *hostmem)
{
    size_t t_pitch = 0;
    width = w;
    height = h;
    pitch = p;
    d_data = devmem;
    h_data = (float *)hostmem;
    t_data = NULL;
    if (devmem == NULL)
    {
        safeCall(cudaMallocPitch((void **)&d_data, (size_t *)&t_pitch, (size_t)(sizeof(float) * width), (size_t)height));
        t_pitch /= sizeof(float);
        pitch = (int)t_pitch;
