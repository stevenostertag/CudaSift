...
inline void __safeCall(cudaError err, const char *file, const int line)
{
    if (cudaSuccess != err)
    {
        std::string errMsg = std::string(cudaGetErrorString(err)) + " in " + file + " at line " + std::to_string(line);
...
inline void __checkMsg(const char *errorMessage, const char *file, const int line)
{
    ...
    if (cudaSuccess != err)
    {
        std::string errMsg = std::string(errorMessage) + " in " + file + " at line " + std::to_string(line);
        throw std::runtime_error(errMsg.c_str());
    }
}
...
inline bool deviceInit(int dev)
{
    ...
    {
        spdlog::error("error: device does not support CUDA.");
        return false;
    }
